...
export function ChallengeForge() {
  ...
  async function freeze() {
    ...
    try {
      const response = await fetch("/api/challenges/freeze", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ specification: compiled.specification }) });
      ...
      <div className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
        ...
        <section className="space-y-6" aria-label="Compiler output">
          {!compiled ? <div className="flex min-h-80 items-center justify-center rounded-3xl border-2 border-dashed border-slate-300 bg-white p-8 text-center text-slate-500">Compile the brief to see the structured specification, deterministic findings, and human approval gate.</div> : <>
            ...
            <article className="rounded-3xl border border-emerald-200 bg-emerald-50 p-5 md:p-6"><p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-800">4 · Human authorization</p><h2 className="mt-2 text-2xl font-black">Freeze, never auto-publish</h2><p className="mt-2 text-sm leading-6 text-slate-700">Freezing locks the reviewed eligibility, rubric, metric and milestone definitions behind a SHA-256 content hash. The server records the signed-in demo user and verifies an active government membership; names and roles are never accepted from this form.</p><label className="mt-4 flex cursor-pointer items-start gap-3 rounded-xl bg-white p-4 text-sm leading-6"><input type="checkbox" checked={humanApproved} onChange={(event) => setHumanApproved(event.target.checked)} className="mt-1 size-5 accent-emerald-800" /><span><strong>I performed the human procurement review.</strong><br />I confirm the deterministic recommendations were reviewed and authorize freezing this demo version. This local confirmation is not an identity or role claim.</span></label><button type="button" onClick={() => void freeze()} disabled={busy || !humanApproved || compiled.findings.length > 0} className="mt-4 min-h-12 w-full rounded-xl bg-slate-950 px-5 py-3 font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">Freeze approved version</button>{frozen && <div className="mt-4 rounded-2xl border border-emerald-300 bg-white p-4" aria-live="polite"><div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-emerald-900">FROZEN · NOT PUBLISHED</strong><span className="text-xs font-bold">SIMULATED_FOR_DEMO</span></div><p className="mt-2 text-sm">Approved by {frozen.approvedBy}. {frozen.notice}</p><p className="mt-3 break-all rounded-lg bg-slate-950 p-3 font-mono text-xs text-emerald-200"><span className="text-slate-400">SHA-256 </span>{frozen.contentHash}</p></div>}</article>
          </>}
        </section>
      </div>
    </div>
